(function(){
  ...
  window.makeurl = function(arr){
    ...
    return window.baseurl+"netroy/index.php?"+df.join("&");
  }
...
$(function(){
  ...
  if (window.noautorefresh)
	  window.tweetUrl = "";
  else if ((window.widgetUrl)&&(window.twtcnt>0))
	  window.tweetUrl = window.makeurl({"r":"main/widgettweets","user":window.user,"list":window.list,"hours":window.hours,"twtcnt":window.twtcnt});
  else if (window.widgetUrl)
...
function linkify(text){
  ...
  text = text.replace(/(https?:\/\/[^\s]+)/i,"<a href='$1' class='ext' target='_blank' rel='nofollow' onClick='_gaq.push([\"_trackEvent\", \"PartnerTweetAction\", \"TweetLinkClick\", \"$1\"],[\"b._trackEvent\", \"PartnerTweetAction\", \"TweetLinkClick\", \"$1\"]);'>$1</a>");
  text = text.replace(/(^|[\s,\.])@([A-Za-z0-9_]+)/g, "$1@<a href=\"http://twitter.com/$2\" target='_blank' onClick='_gaq.push([\"_trackEvent\", \"PartnerTweetAction\", \"TweetUserProfileClick\", \"$2\"],[\"b._trackEvent\", \"PartnerTweetAction\", \"TweetUserProfileClick\", \"$2\"]);'>$2</a>");
  text = text.replace(/(^|[\s,\.])#([A-Za-z0-9_]+)/g,
    "$1#<a href=\"http://search.twitter.com/search?had_popular=true&q=$2\" target='_blank' onClick='_gaq.push([\"_trackEvent\", \"PartnerTweetAction\", \"TweetHashTagClick\", \"$2\"],[\"b._trackEvent\", \"PartnerTweetAction\", \"TweetHashTagClick\", \"$2\"]);'>$2</a>");
  return text;
}
...
function replyTweet(e){
  ...
  _gaq.push(['_trackEvent', 'PartnerTweetAction', 'Reply', id],
            ['b._trackEvent', 'PartnerTweetAction', 'Reply', id]);
  if(typeof window.loggedinuser == 'undefined'){
    modal.info("not logged in... login with twitter to post reply and retweet");
    $.oauthpopup({path: '/partner/login?partner='+window.partnerid,callback: function(){
      doReplyTweet(parent, id, function() { window.location.reload();});
    }});
    return false;
  }
  doReplyTweet(parent, id);
}

function doReplyTweet(parent, id, callback){
  var usr= $("span.user",parent).html();
  modal.prompt("replying to "+usr,"@"+usr+" ",function(msg){
    modal.loader.show("posting your reply");
    $.get(baseurl+"partner/reply",{tweet_id:id,tweet_text:msg,partner:window.partnerid},function(data){
      if(data.response==200) {
        modal.info("reply posted");
...
function retweet(e){
  ...
  var id = parent.attr("id").substr(2);
  _gaq.push(['_trackEvent', 'PartnerTweetAction', 'Retweet', id],
            ['b._trackEvent', 'PartnerTweetAction', 'Retweet', id]);
  if(typeof window.loggedinuser == 'undefined'){
    modal.info("not logged in... login with twitter to post reply and retweet");
    $.oauthpopup({path: '/partner/login?partner='+window.partnerid,callback: function(){
      doRetweetTweet(parent, id, function(){window.location.reload();});
    }});
    return false;
  }
  doRetweetTweet(parent, id);
}
function doRetweetTweet(parent, id, callback){
  modal.confirm("do you want to retweet this?",function(){
    modal.loader.show("posting the retweet");
    $.get(baseurl+"partner/retweet",{tweet_id:id,partner:window.partnerid},function(data){
      if(data.response==200){ 
        modal.info("retweet posted");
        if (typeof callback == 'function')
          callback();
      }
      else if(data.response==400) modal.info("not logged in");
